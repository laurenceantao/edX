...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for j in range(numTrials):
        ...
        for k in range(150):
            ...
            avgViruses[k] += upd
            gResistantViruses[k] += resist
            
        patient.addPrescription('guttagonol')
        
        for l in range(150,300):
            upd2 = patient.update()
            resist2 = patient.getResistPop('guttagonol')
            avgViruses[l] += upd2
            gResistantViruses[l] += resist2
    for m in range(300):
        avgViruses[m] /= numTrials
        gResistantViruses[m] /= numTrials
    
    pylab.title('ResistantVirus simulation')
    pylab.xlabel('time step')
    pylab.ylabel('# viruses')
    pylab.plot(avgViruses, label='total')
    pylab.plot(gResistantViruses, label='resistant')
    pylab.legend()
    pylab.show()
